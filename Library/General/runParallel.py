...
def call_script(run_dir, robot_test_suite, robot_output, tag, lang, browser, take_timeout, n_type, run_name):
    ...
    command = "pybot --nostatusrc -t \"" + run_name + "\" -d " + robot_output + " -o output" + full_output_name + " -r report" + full_output_name \
              + " -l log" + full_output_name + " -v ar_LANG:" + lang + " -v ar_BROWSER:" + browser \
              + " -v ar_TAKE_TIMEOUT:" + take_timeout + " -v ar_NType:" + str(n_type) + " " + tag + " " + robot_test_suite
...
def parallel_execution(robot_path, robot_test_suite, robot_output, tag="", lang="EN", browser="gc", take_timeout="auto", n_type_support=[]):
    print "begin"
    ...
    if len(n_type_support) == 0:
        ...
        n_type = ""
        func = partial(call_script, run_dir, robot_test_suite, robot_output, tag, lang, browser, take_timeout, n_type)
        pool.map_async(func, run_name)
